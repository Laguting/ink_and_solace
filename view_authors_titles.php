...
<?php if ($has_results): ?>
    ...
    <script>
        function openDetails(data) {
            ...
            // Removed au_minit/v_minit to prevent crashes
            const map = {
                'v_au_id': data.au_id, 
                'v_lname': data.au_lname, 
                'v_fname': data.au_fname,
                'v_phone': data.phone, 
                'v_addr': data.address, 
                'v_city': data.city, 
                'v_state': data.state, 
                'v_zip': data.zip, 
                'v_contract': data.contract,
                'v_title_id': data.title_id, 
                'v_title': data.title, 
                'v_type': data.type, 
                'v_pub_id': data.pub_id, 
                'v_price': data.price,
                'v_advance': data.advance, 
                'v_royalty': data.royalty, 
                'v_ytd': data.ytd_sales, 
                'v_notes': data.notes, // Ensure this maps to the notes field
                'v_pubdate': data.pubdate
            };

            for(let id in map) {
                const element = document.getElementById(id);
                if(element) {
                    // If the data is null/undefined, show a dash instead of blank
                    element.innerText = (map[id] !== null && map[id] !== undefined) ? map[id] : '-';
                }
            }
        }
